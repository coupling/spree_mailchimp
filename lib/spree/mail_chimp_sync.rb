  ...
  module Sync
      ...
      def self.included(target)
          target.class_eval do
              ...
              #TODO remove_from_mailchimp

              def hominid
                  @hominid ||= Hominid::Base.new(Spree::MailChimp::Config[:mailchimp_api_key])
              end

              def mc_list_id
                  Spree::MailChimp::Config.get(:mailchimp_list_id)
              end
          end
      end
      ...
      def create_in_mailchimp
          ...
          User.benchmark "Adding mailchimp subscriber (list id=#{mc_list_id})" do
              hominid.subscribe(mc_list_id, @user.email, mc_merge_vars, Spree::MailChimpSync::Sync::mc_subscription_opts)
          end
         ...
         logger.warn "Spree::MailChimp::Sync: Failed to create contact #{id} in mailchimp: #{e.message}"
      end
      ...
      def update_in_mailchimp
       ...
       logger.warn "Spree::MailChimp::Sync: Failed to update mailchimp record for user id=#{@user.id}"
      end
      ...
      def mc_merge_vars
          merge_vars = {}
          if mailchimp_merge_user_attribs = Spree::MailChimp::Config.get(:mailchimp_merge_vars)
      ...
      def self.mc_subscription_opts
          ...
          [:mailchimp_double_opt_in, :mailchimp_send_welcome, :mailchimp_send_notify].each do |opt|
            puts "opt = #{opt}"
              options[opt.to_s.gsub(/^mailchimp_/,'')] = Spree::MailChimp::Config.get(opt)
            puts "opt2 = #{opt}"
          end
          options
      end
